...
export default function CrudManager({ title, apiEndpoint, fields, idKey = "id" }: CrudManagerProps) {
  ...
  function renderCell(item: Record<string, unknown>, field: FieldDef) {
    ...
    if (isImageField(field)) {
      ...
      if (src) {
        return (
          <div className="w-10 h-10 rounded-lg overflow-hidden border border-[#1EDAC6]/20 relative">
            <Image src={src} alt="" fill sizes="40px" className="object-cover" />
          </div>
        );
      }
      return (
        <div className="w-10 h-10 rounded-lg bg-card border border-[#1EDAC6]/10 flex items-center justify-center">
    ...
    <div className="min-h-screen bg-background text-white">
      <nav className="border-b border-[#1EDAC6]/20 bg-surface-raised">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <button
            onClick={openCreate}
            className="flex items-center gap-2 px-3 py-1.5 bg-[#1EDAC6] hover:bg-[#34F5E2] text-black text-sm font-semibold rounded-lg transition-colors"
          >
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        {loading ? (
          ...
          <div className="overflow-x-auto rounded-xl border border-[#1EDAC6]/10">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-surface-raised border-b border-[#1EDAC6]/10 text-left text-gray-400">
                  {displayFields.map((f) => (
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-surface-raised border border-[#1EDAC6]/20 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleSave} className="p-5 space-y-4">
                ...
                <div key={f.key}>
                  ...
                  {isImageField(f) ? (
                    <div className="space-y-3">
                      <div className="relative">
                        ...
                        <label
                          ...
                          className="flex items-center gap-2 px-3 py-2 bg-card border border-dashed border-[#1EDAC6]/30 rounded-lg text-sm text-gray-400 hover:text-white hover:border-[#1EDAC6] cursor-pointer transition-colors"
                        >
                      ...
                      {Boolean(form[f.key]) && getImageSrc(form[f.key]) && (
                        <div className="rounded-lg overflow-hidden border border-[#1EDAC6]/20 w-full max-w-[200px] relative aspect-[5/4]">
                          <Image
                    ...
                    <textarea
                      value={String(form[f.key] ?? "")}
                      onChange={(e) => setForm((prev) => ({ ...prev, [f.key]: e.target.value }))}
                      className="w-full px-3 py-2 bg-card border border-[#1EDAC6]/20 rounded-lg text-white text-sm focus:outline-none focus:border-[#1EDAC6] resize-none"
                      rows={3}
                    />
                  ) : f.type === "json" ? (
                    <textarea
                    ...
                    <label className="inline-flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        className="w-4 h-4 accent-[#1EDAC6]"
                      />
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="px-4 py-2 bg-[#1EDAC6] hover:bg-[#34F5E2] text-black text-sm font-semibold rounded-lg transition-colors disabled:opacity-50"
                >
